...
class MY_Model extends CI_Model
{
    ...
    //constructor pintar menebak nama model 
    public function __construct()
    {
        ...
        if (!$this->_tabel) {
            //huruf diubah jadi kecil cari _model diganti dengan kosong cara mencarinya menggunakan get_class
            $this->_tabel = 'tb_' . strtolower(str_replace('_model', '', get_class($this)));
        }
    }

    // Ambil 1 record, menerima beberapa pola "where"
    // fungsi search untuk dari keyword
    public function get()
    {
    ...
    {
        $data = (object)$data;
        $data->created_at = $data->update_at = date('Y:m:d H:i:s');
